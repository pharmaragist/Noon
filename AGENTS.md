...
- `NoonUtils.*`: `toast(...)`, `notify(content, title)`, `playSound(sound)`, `execDetached(command, log?)`, `iconPath(icon, fallback)`, `openFile(path)`, `deleteFile(path)`, `setHyprKey(key, value)`, `requestDialog(dialog, data)`, `inlineTimer(callback, delay)`.
