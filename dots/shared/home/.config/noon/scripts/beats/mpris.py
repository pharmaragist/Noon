...
class MprisPlayer(ServiceInterface):
    ...
    @dbus_property(access=PropertyAccess.READ)
    def Position(self) -> 'x':
        return int(self._snap.get('position', 0) * 1000000)
    ...
    def Seek(self, offset: 'x'):
        self._player.seek(offset / 1000000, relative=True)
        self._emit_seeked()

    @method()
    def SetPosition(self, playlist_id: 'o', position: 'x'):
        self._player.seek(position / 1000000, relative=False)
        self._emit_seeked()
    ...
    def _emit_seeked(self):
        self._refresh()
        self.Seeked(int(self._snap.get('position', 0) * 1000000))
    ...
    async def _sync(self):
        ...
        while True:
            try:
                iface._refresh()
                s = iface._snap
                changed = {'PlaybackStatus': _MP_STATUS.get(s.get('state'), 'Stopped'), 'LoopStatus': iface._loop_status(), 'Shuffle': s.get('random', False), 'Rate': round(s.get('rate', 1.0), 3), 'Metadata': iface._meta(), 'Volume': round(s.get('volume', 0) / 100.0, 3)}
                if s.get('state') != 'stop':
                    changed['Position'] = int(s.get('position', 0) * 1000000)
                if changed != last:
                    last = changed
                    iface.emit(changed)
            except Exception:
