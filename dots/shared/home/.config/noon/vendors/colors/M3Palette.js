function parseColor(value) {
  ...
  if (typeof value !== "string") return 0xffffffff;
  let s = value.trim();
  if (s[0] === "#") s = s.slice(1);
...
class TemperatureCache {
  ...
  get hctsByTemp() {
    ...
    const hcts = this.hctsByHue
      .concat([this.input])
      .sort((a, b) => temps.get(a) - temps.get(b));
  ...
  relativeTemperature(hct) {
    ...
    return (
      (this.tempsByHct.get(hct) - this.tempsByHct.get(this.coldest)) / range
    );
  }
  get complement() {
    if (this.complementCache != null) return this.complementCache;
    const coldest = this.coldest,
      warmest = this.warmest;
    const coldestTemp = this.tempsByHct.get(coldest),
      warmestTemp = this.tempsByHct.get(warmest);
    ...
    for (let hueAddend = 0.0; hueAddend <= 360.0; hueAddend += 1.0) {
      ...
      const relativeTemp =
        (this.tempsByHct.get(this.hctsByHue[Math.round(hue)]) - coldestTemp) /
        range;
  ...
  analogous(count = 5, divisions = 12) {
    ...
    while (allColors.length < divisions) {
      ...
      while (indexSatisfied && allColors.length < divisions) {
        allColors.push(hct);
        indexSatisfied =
          totalTempDelta >= (allColors.length + indexAddend) * tempStep;
        indexAddend++;
      }
      lastTemp = temp;
      hueAddend++;
    ...
    for (let i = 1; i < increaseHueCount + 1; i++)
      answers.splice(
        0,
        0,
        allColors[(allColors.length - i) % allColors.length],
      );
...
function fixIfDisliked(hct, Hct) {
  const passes =
    Math.round(hct.hue) >= 90.0 &&
    Math.round(hct.hue) <= 111.0 &&
    Math.round(hct.chroma) > 16.0 &&
    Math.round(hct.tone) < 65.0;
  ...
  switch (scheme) {
    ...
    case "scheme-expressive":
      return {
        ...
        a2: T(
          getRotatedHue(hct, EXPRESSIVE_HUES, EXPRESSIVE_SECONDARY_ROTATIONS),
          24,
        ),
        a3: T(
          getRotatedHue(hct, EXPRESSIVE_HUES, EXPRESSIVE_TERTIARY_ROTATIONS),
          32,
        ),
    ...
    case "scheme-fidelity": {
      ...
      return {
        ...
        a3: m3.TonalPalette.fromInt(
          fixIfDisliked(cache.complement, m3.Hct).toInt(),
        ),
        n1: T(h, c / 8),
        n2: T(h, c / 8 + 4),
      };
    ...
    case "scheme-content": {
      ...
      return {
        ...
        a2: T(h, Math.max(c - 32, c * 0.5)),
        a3: m3.TonalPalette.fromInt(
          fixIfDisliked(cache.analogous(3, 6)[2], m3.Hct).toInt(),
        ),
        n1: T(h, c / 8),
        n2: T(h, c / 8 + 4),
      };
    ...
    case "scheme-rainbow":
      return {
        a1: T(h, 48),
        a2: T(h, 16),
        a3: T(sanitizeDegreesDouble(h + 60), 24),
        n1: T(h, 0),
        n2: T(h, 0),
      };
    ...
    case "scheme-vibrant":
      return {
        a1: T(h, 200),
        a2: T(
          getRotatedHue(hct, VIBRANT_HUES, VIBRANT_SECONDARY_ROTATIONS),
          24,
        ),
