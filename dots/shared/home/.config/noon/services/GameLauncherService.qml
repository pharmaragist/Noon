...
Singleton {
    ...
    property var launchConfig: ({
            ...
            vkd3d: true,
            mangohud: false,
            extraEnv: ["__NV_PRIME_RENDER_OFFLOAD=1", "__GLX_VENDOR_LIBRARY_NAME=nvidia", "DXVK_ASYNC=1", "DXVK_FRAME_RATE=0", "DXVK_STATE_CACHE=1", "WINEFSYNC=1", "STAGING_SHARED_MEMORY=1", "WINE_LARGE_ADDRESS_AWARE=1", "__GL_SHADER_DISK_CACHE=1", "__GL_SHADER_DISK_CACHE_SKIP_CLEANUP=1", "DXVK_LOG_LEVEL=none"],
            wineArgs: []
        })

    Component.onCompleted: resetStatus()
