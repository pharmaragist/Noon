...
(async function comfy() {
  ...
  let configScheme = Spicetify.Config.options?.color_scheme || "Comfy";
  ...
  fetch(
    ...
    .then((iniContent) => {
      setConfig(
        "Color-Schemes",
        parseIni(iniContent),
        "Successfully updated color schemes",
      );
      configScheme =
        (getConfig("Color-Schemes") &&
          Object.keys(getConfig("Color-Schemes")).find(
            (scheme) => scheme.toLowerCase() === configScheme.toLowerCase(),
          )) ||
        configScheme;
  ...
  // Layout Variables
  let panelWidth =
    document.documentElement.style.getPropertyValue("--panel-width");
  let sidebarWidth = document.documentElement.style.getPropertyValue(
    "--left-sidebar-width",
  );
  ...
  new MutationObserver((mutations) => {
    const newPanelWidth =
      mutations[0].target.style.getPropertyValue("--panel-width");
    const newSidebarWidth = mutations[0].target.style.getPropertyValue(
      "--left-sidebar-width",
    );
    if (newPanelWidth !== panelWidth || newSidebarWidth !== sidebarWidth) {
      ...
      document.documentElement.style.setProperty(
        "--comfy-panel-width",
        `${trimmedPanelWidth}px`,
      );
      document.documentElement.style.setProperty(
        "--comfy-left-sidebar-width",
        `${trimmedSidebarWidth}px`,
      );
  ...
  waitForDeps("Spicetify.Platform.History", () =>
    Spicetify.Platform.History.listen(updateBanner),
  );
  waitForDeps("Spicetify.Player", () =>
  ...
  const CardLayout = Spicetify.React.memo(
    ({ title, desc, tippy, action, onClick }) => {
      return Spicetify.React.createElement(
        "div",
  ...
  const SubSection = Spicetify.React.memo(
    ({
      name,
      condition = true,
      items,
      collapseItems: initialCollapseItems = false,
      callback,
      ...props
    }) => {
      ...
      const [collapseItems, setCollapseItems] = Spicetify.React.useState(
        getConfig(`${name}-Collapsed`) ?? initialCollapseItems,
      );
  ...
  const Input = Spicetify.React.memo(
    ({
      ...
      const [defaultState, setDefaultState] =
        Spicetify.React.useState(defaultVal);
      const isFirstRender = Spicetify.React.useRef(true);
  ...
  const Dropdown = Spicetify.React.memo(
    ({
      ...
      const [selectedValue, setSelectedValue] = Spicetify.React.useState(
        getConfig(name) ?? defaultVal,
      );
      const [buttonEnabled, setButtonEnabled] = Spicetify.React.useState(
        ...
        if (isFirstRender.current) {
          ...
          if (!startup) {
            const parent = document.querySelector(
              "generic-modal [aria-label='Comfy Settings']",
            );
  ...
  const Carousel = ({ chips, checked, setChecked }) => {
    ...
    const [showRightButton, setShowRightButton] =
      Spicetify.React.useState(false);
    ...
    Spicetify.React.useEffect(() => {
      if (containerRef.current && isFirstRender.current) {
        const activeItem = containerRef.current.querySelector(
          `.search-searchCategory-categoryGridItem:nth-child(${checked.index + 1})`,
        );
        if (activeItem) {
          const container = containerRef.current;
          ...
          if (
            itemOffsetLeft < containerScrollLeft + 32 ||
            itemOffsetLeft + itemWidth >
              containerScrollLeft + containerWidth - 32
          ) {
      ...
      (event) => {
        ...
        if (
          document.activeElement.classList.contains(
            "search-searchCategory-categoryGridItem",
          )
        ) {
          startingIndex = Array.from(
        ...
        if (newIndex !== undefined) {
          const chipElement = containerRef.current.querySelector(
            `.search-searchCategory-categoryGridItem:nth-child(${newIndex + 1})`,
          );
    ...
    const clickCallback = (index, label) => {
      if (scrollLeft === containerRef.current.scrollLeft)
  ...
  const Content = () => {
    const defaultFilter = sessionStorage.getItem("comfy-settings-filter")
      ? JSON.parse(sessionStorage.getItem("comfy-settings-filter"))
      : null;
    const [filter, setFilter] = Spicetify.React.useState(
      defaultFilter ?? { index: 0, label: "All" },
    );
    ...
    return Spicetify.React.createElement(
      ...
      Spicetify.React.createElement(Section, { name: "Colorscheme", filter }, [
        {
          ...
          condition:
            ...
            !document.querySelector(
              "body > style.marketplaceCSS.marketplaceScheme",
            ),
          callback: (name, value) => {
            updateScheme(value);
          },
            ...
            waitForDeps(
              ...
              (main) => {
                main.classList.remove(
                  ...options.map((option) => `Comfy-${option}-Snippet`),
                );
                if (value !== defaultVal)
                  main.classList.add(`Comfy-${value}-Snippet`);
              },
              true,
              "getElementById",
            );
      ...
      Spicetify.React.createElement(Section, { name: "Interface", filter }, [
        ...
        },
        {
          ...
          defaultVal: false,
          condition:
            Spicetify.Platform.version <= "1.2.46.462" &&
            !document.querySelector(":root .global-nav"),
          callback: (value) => {
              ...
              callback: (value) => {
                ...
                if (isValidUrl(value)) {
                  fontFamily = decodeURIComponent(
                    value.match(/family=([^&:]+)/)?.[1]?.replace(/\+/g, " "),
                  );
        ...
        },
        {
          ...
          tippy: Spicetify.React.createElement(
            ...
            Spicetify.React.createElement(
              "h4",
              null,
              "Set to 0 to disable the gradient!",
            ),
          ),
          callback: (value) =>
            document.documentElement.style.setProperty(
        ...
        },
        {
          ...
          tippy: Spicetify.React.createElement(
            ...
            Spicetify.React.createElement(
              "h4",
              null,
              "Set to 0 for no gradient color!",
            ),
          ),
          callback: (value) =>
            document.documentElement.style.setProperty(
        ...
        },
        {
          ...
          callback: (value) => {
            waitForDeps(
              ".Root__top-container",
              (topContainer) => {
                const playbar = topContainer.querySelector(
                  ".Root__now-playing-bar",
                );

                if (value) {
                  const rightbar = topContainer.querySelector(
                    ".Root__right-sidebar",
                  );
      ...
      Spicetify.React.createElement(Section, { name: "Cover Art", filter }, [
        {
          ...
          tippy: Spicetify.React.createElement(
            ...
            Spicetify.React.createElement(
              "h4",
              null,
              "Change the size of the cover art:",
            ),
            Spicetify.React.createElement(
              "li",
              null,
              "Comfy default: (84px, 84px, 8px, 20px)",
            ),
            Spicetify.React.createElement(
              "li",
              null,
              "Spotify default: (56px, 56px, 4px, 0px)",
            ),
            Spicetify.React.createElement(
              "li",
              null,
              "Oblong: (115px, 84px, 15px, 20px)",
            ),
          ),
          items: [
            {
            ...
            },
            {
              ...
              tippy: Spicetify.React.createElement(
                ...
                Spicetify.React.createElement(
                  "h4",
                  null,
                  "Width of circles in relation to viewport (in %)",
                ),
          ...
          items: [
            {
              ...
              tippy: Spicetify.React.createElement(
                ...
                Spicetify.React.createElement(
                  "li",
                  null,
                  "Enter any raw image url into text box, e.g. 'https://example.com/image.png'",
                ),
                Spicetify.React.createElement("h4", null, "Local Images:"),
                Spicetify.React.createElement(
                  "li",
                  null,
                  "Place desired image in 'spotify/Apps/xpui/images'",
                ),
                Spicetify.React.createElement(
                  "li",
                  null,
                  "Enter 'images/image.png' into text box",
                ),
              ),
              callback: updateBanner,
            },
        ...
        },
        {
          ...
          tippy: Spicetify.React.createElement(
            ...
            Spicetify.React.createElement(
              "h4",
              null,
              "Amount of banner blur in pixels:",
            ),
      ...
      Spicetify.React.createElement(Section, { name: "Settings", filter }, [
        {
          ...
          callback: async (value) => {
            waitForDeps("Spicetify.Platform.UserAPI", async () => {
              ...
              fetch(
                `https://included-exotic-javelin.ngrok-free.app/${endpoint}`,
                {
                  method: "POST",
                  headers: {
                ...
                .catch((error) =>
                  console.warn(
                    "[Comfy-Warning]: Failed to send/purge analytics:",
                    error,
                  ),
                );
            });
            ...
            },
            {
              ...
              callback: (state, setState) => {
                Spicetify.Platform.ClipboardAPI.copy(
                  localStorage.getItem("comfy:config"),
                );
                setState("Copied!");
            ...
            },
            {
              ...
              callback: () => {
                const settings = document.querySelector(
                  ".GenericModal__overlay:has(.comfy-settings)",
                );
  ...
  waitForDeps("Spicetify.Topbar.Button", () => {
    new Spicetify.Topbar.Button(
      ...
      () => {
        ...
        document
          .querySelector(".main-trackCreditsModal-closeBtn[aria-label='Close']")
          .addEventListener("click", function () {
            document.getElementById("main").classList.remove("Settings-Open");
          });

        document
          .querySelector("generic-modal")
        ...
        const scrollVal = cache
          ? cache * (section.scrollHeight - section.clientHeight)
          : 0;
      ...
      async function checkElements() {
        const check = () =>
          Array.isArray(dependencies)
            ? dependencies.every((element) => document[elementType](element))
            : document[elementType](dependencies);
        if (check()) {
          callback?.(
            Array.isArray(dependencies)
              ? dependencies.map((d) => document[elementType](d))
              : document[elementType](dependencies),
          );
          resolve();
        } else {
        ...
        for (const dependency of Array.isArray(dependencies)
          ...
          if (!eval(dependency)) {
            if (Date.now() - startTime < timeout) {
              ...
              console.error(
                `[Comfy-Error]: Dependency Timeout -`,
                dependencies,
              );
              resolve();
            }
  ...
  async function updateBanner() {
    ...
    if (
      getConfig("Apple-Music-Gradient-Snippet") &&
      getConfig("AM-Gradient-Include-Existing-Snippet")
    ) {
      const [isPlaylist, isArtist] = [
      ...
      if (isPlaylist || isArtist) {
        ...
        source = isPlaylist
          ? metadata.images[3]?.url
          : metadata.data.artistUnion.visuals.headerImage?.sources?.[0]?.url;
      }
    }
    ...
    if (!source && getConfig("Prefer-Existing-Image")) {
      if (Spicetify.URI.isPlaylistV1OrV2(pathname)) {
        ...
        const album = await Spicetify.CosmosAsync.get(
          `https://api.spotify.com/v1/albums/${pathname.split("/").pop()}`,
        );
    ...
    const validChannel = Object.values(channels).some(
    ...
    if (
      document.documentElement.style.getPropertyValue("--image-url") !==
      `url(${source})`
    ) {
  ...
  function updateScheme(scheme, message) {
    ...
    if (colorSchemes[scheme] && !marketplace && scheme !== configScheme) {
      console.debug(
        `[Comfy-Event]: Scheme ${message ? message : "applied"} - ${scheme}`,
      );
